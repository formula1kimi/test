...
static void int_exit(int sig)
{
	...
	if (bpf_xdp_query_id(ifindex_in, xdp_flags, &curr_prog_id)) {
	...
	if (ifindex_out_xdp_dummy_attached) {
		curr_prog_id = 0;
		if (bpf_xdp_query_id(ifindex_out, xdp_flags, &curr_prog_id)) {
			printf("bpf_get_link_xdp_id failed\n");
			exit(1);
		}
		if (dummy_prog_id == curr_prog_id)
			bpf_xdp_attach(ifindex_out, -1, xdp_flags, NULL);
		else if (!curr_prog_id)
...
int main(int argc, char **argv)
{
	...
	prog = bpf_object__find_program_by_name(obj, "xdp_redirect_prog");
	...
	if (bpf_xdp_attach(ifindex_in, prog_fd, xdp_flags, NULL) < 0) {
	...
	if (bpf_xdp_attach(ifindex_out, dummy_prog_fd,
			    (xdp_flags | XDP_FLAGS_UPDATE_IF_NOEXIST), NULL) < 0) {

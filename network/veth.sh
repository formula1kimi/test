...
ip a a 10.10.50.1/24 dev veth1
...
# 10.10.50.0的报文走veth1
ip route del 10.10.50.0/24 dev veth1 src 10.10.50.1
ip route add 10.10.50.0/24 dev veth1 src 10.10.50.1  metric 100
# 更新自动添加的veth0上的路由的metric，优先veth1
ip route del 10.10.50.0/24 dev veth0 src 10.10.50.2
ip route add 10.10.50.0/24 dev veth0 src 10.10.50.2 metric 200

# 允许veth1和eth0间的报文转发
iptables -w -t filter -A FORWARD -i veth1 -o eth0 -j ACCEPT
iptables -w -t filter -A FORWARD -i eth0 -o veth1 -j ACCEPT
# 当从veth0发送报文时，做SNAT到10.10.50.3
iptables -w -t nat -A POSTROUTING -o veth0 -j SNAT --to-source 10.10.50.3
# 当从veth1出来的报文转发到eth0时，做MASQUERADE
iptables -w -t nat -A POSTROUTING -s 10.10.50.3/32 -o eth0 -j MASQUERADE

# 告诉veth1，10.10.50.3的MAC地址为veth0（10.10.50.3没有对应实际接口，ARP无法找到它）
MAC=$(ip -br link show dev veth0 | awk '{print $3}')
ip neigh add 10.10.50.3 lladdr ${MAC} dev veth1  nud permanent
...
ping -I veth0 114.114.114.114 -c 1
